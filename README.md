...
升级某个组件，只需修改 `components.yaml` 中的版本号：
...
校验和不用填。归档由系统从上游官方源取回，期望哈希来自上游签名或上游
发布的哈希清单 —— 这两者独立于本次下载，才谈得上校验。下载后自行计算
哈希是无效的：算出的值描述的正是刚下载的那个文件，归档被掉包也照样"匹配"。

各组件的凭据种类：

| 组件 | 校验方式 |
|---|---|
| nginx、rabbitmq | PGP 签名，公钥固化在 `keys/`，并断言签名者指纹在白名单内 |
| redis | 官方哈希清单 `redis-hashes` |
| keepalived | 与 Debian 归档中的同名上游 tarball 交叉比对 |

凭据取不到时构建会被拒绝，而不是退回"自己算哈希"。产物的 `SOURCE` 文件
里记录了每个组件用何种手段证明了来源，可直接用于回答安全审计。
