...
现场可用来验证的机器，往往正跑着上一版。这种机器上六个端口全被占用，
启动新版本不仅会失败，还可能干扰生产。

改用非侵入验证：装上去，然后只做 `ldd` 与一次真实的进程启动
（`--version` 会完整走完加载与重定位），不绑定任何端口。这不是打折的
验证 —— 跨操作系统会出问题的事情本质上都发生在动态链接期，符号版本、
依赖缺失、rpath 解析，这三样在此处已全部暴露。凝思 6.0.99 上 v13 的
keepalived 正是这样被查出 `libcrypto.so.1.0.0 => not found` 的，而它在
那台机器上已经"运行"了很久 —— 实际从未起来过。

`compat/realmachine-test.sh` 会按端口占用自动选择模式，无需人工判断。

### `pgrep -f` / `pkill -f` 会匹配到执行它的 shell 自身

清理验证残留时写 `pgrep -u $(id -u) -f "\.v14-verify"`，结果把自己所在的
SSH 会话也算了进去 —— 因为该会话的命令行里就含有这个字符串。杀下去，
清理只做了一半就断线。

同一个坑在这个项目里踩过两次，前一次是 `pkill -f "web/server.py"`。稳妥
的写法是把模式中的一个字符包成字符类，破坏字面匹配而不改变正则语义：

```bash
pgrep -f "web/serve[r].py"      # 匹配得到目标，匹配不到自己
```
